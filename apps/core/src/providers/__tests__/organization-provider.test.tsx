...
const { getActiveOrgCookieName } = jest.requireActual(
  '@/hooks/use-stored-settings',
)
...
describe('OrganizationProvider Security Tests', () => {
  ...
  describe('Server-side Props Integration', () => {
    ...
    it('should prioritize fetched active organization over organizations array', () => {
      ...
      expect(result.current.selectedOrganization?.avatar).toBe(
        'fetched-avatar-url',
      )
      ...
      expect(result.current.selectedOrganization?.avatar).not.toBe(
        orgFromArray?.avatar,
      )
    })
  })
      ...
      it('should pass initialActiveOrgId to useStoredSettings', () => {
        const wrapper = ({ children }: { children: React.ReactNode }) => (
          <QueryClientProvider client={new QueryClient()}>
            <OrganizationProvider
              userId='user-123'
              initialActiveOrgId='server-org-id'
            >
              {children}
            </OrganizationProvider>
          </QueryClientProvider>
      ...
      it('should not have hydration mismatches with consistent server-client data', () => {
        ...
        const wrapper = ({ children }: { children: React.ReactNode }) => (
          <QueryClientProvider client={new QueryClient()}>
            <OrganizationProvider
              userId='user-123'
              initialActiveOrgId='consistent-org-id'
            >
              {children}
            </OrganizationProvider>
          </QueryClientProvider>
      ...
      it('should handle special characters in organization IDs', () => {
        ...
        const wrapper = ({ children }: { children: React.ReactNode }) => (
          <QueryClientProvider client={new QueryClient()}>
            <OrganizationProvider
              userId='user-123'
              initialActiveOrgId={specialOrgId}
            >
              {children}
            </OrganizationProvider>
          </QueryClientProvider>
